...
-- himedia
-- SIMPLE VIEW
-- 단일 테이블 혹은 단일 뷰를 베이스로 한 함수, 연산식을 포함하지 않는 뷰 (JOIN은 가능)

-- emp123
DESC emp123;

-- emp123 테이블을 기반으로 department_id =10 부서 소속 사원만 조회하는 뷰
...
-- SIMPLE VIEW는 제약 사항에 걸리지 않는 다면 INSERT, UPDATE, DELETE를 할 수 있다
...
-- COMPLEX VIEW
-- 한개 혹은 여러 개의 테이블 혹은 뷰에 JOIN, 함수 연산식 등을 활용한 VIEW
-- 특별한 경우가 아니면 INSERT, UPDATE, DELETE 작업 수행이 불가하다
...
-- VIEW를 포함한 모든 DB 객체들의 정보
SELECT * FROM USER_OBJECTS; 

-- VIEW를 삭제 : DROP VIEW
-- VIEW는 삭제해도 기반 테이블 데이터는 삭제되지 않는다
...
-- 새로운 레코드를 추가하고자 할 때 겹치지 않는 유일한 PK가 필요하다
...
SELECT  my_seq.NEXTVAL FROM dual;  -- 다음 시퀀스를 추출하는 가상 컬럼
